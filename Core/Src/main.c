...
uint32_t val1 = 0;
uint32_t val2 = 0;
uint16_t distance  = 0; // in centimetri
...
void set_pwm_frequency(TIM_HandleTypeDef *htim, uint16_t frequency) {
	...
	// Calcola il valore del registro ARR
	uint32_t arr_value = timer_clock / ( ( psc_value + 1 ) * frequency )  ;

	// Aggiorna il registro ARR del timer
...
int main(void)
{
  ...
  // settiamo il pin TRIG nello stato low
  HAL_GPIO_WritePin(TRIG_PORT, TRIG_PIN, GPIO_PIN_RESET);
  ...
  while (1)
  {
		...
		// Converte il valore di distanza in frequenza della PWM
